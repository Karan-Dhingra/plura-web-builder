...
export const saveActivityLogsNotification = async({ agencyId, description, subaccountId } : { agencyId?: string, description: string, subaccountId?: string }) => {
    ...
    if(!foundAgencyId) {
        if(!subaccountId) {
            throw new Error(`You need to provide atleast one agency Id or subaccount id.`);
        }
...
export const upsertAgency = async (agency: Agency, price?: Plan) => {
    ...
    const passedAgency : Partial<Agency> = {...agency}
    delete passedAgency.id;
...
export const upsertSubAccount = async (subAccount: SubAccount) => {
    ...
    const passedAccount : Partial<Agency> = {...subAccount}
    delete passedAccount.id;

    if (!agencyOwner) return console.log('🔴Erorr could not create subaccount')
    const permissionId = v4()
    const response = await db.subAccount.upsert({
      where: { id: subAccount.id },
      update: passedAccount,
      create: {
        ...subAccount,
        Permissions: {
          create: {
